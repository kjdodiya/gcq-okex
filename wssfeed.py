...
class WSSFeed(Feed):
    ...
    def __init__(self, listing, keyring=None, loop=None, capture_data=False, logger=None): #, lib_ob_flat=None):
        ...
        if listing.exchange.code == 'CEX':
            self.cex = CEX(username=listing.exchange.account,
                           **self.keyring.get_credentials(listing.exchange),
                           loop=loop)
                        ...
                        try:
                            ...
                            try:
                                ...
                                self.logger.exception(
                                    "Error while sending keepalive to %s server: %s" % (
                                        self.listing.exchange.name, e))
                                break  # inner loop
            except Exception as e:
                self.logger.exception(
                    "Exception on websocket %s : %s, reconnecting in %s s" % (
                        self.listing.code, e, self.breather * 20))
                self.breather += 1
    ...
    def process(self, message):
        ...
        if not self.startup_completed:
            self.startup_completed = True
        if self.order_book.best_bid_size < 0 \
                or self.order_book.best_offer_size < 0 \
                or (self.order_book.best_bid > self.order_book.best_offer > 0):
            self.logger.warning(
                'Houston, we have a problem with listing {0}. Best bid: {1} in {2} lots, Best offer: {3} in {4} lots'.format(
                    self.listing.code, self.order_book.best_bid,
                    self.order_book.best_bid_size, self.order_book.best_offer,
                    self.order_book.best_offer_size))
    ...
    def __update_order_book(self, dispatch):
        ...
        try:
            if dispatch.nature == DispatchNature.trade:
                dico = {}
                dico['buy/sell'] = (
                    'buy' if dispatch.action == DispatchAction.lift else 'sell')
                dico['price'] = float(
                    dispatch.price)  # need to cast to float because arctic does not accept decimals
                ...
                if dispatch.action == DispatchAction.bid:
                    if self.order_book.best_bid < dispatch.price:
                        ...
                        if dispatch.price in self.order_book.bids:
                            self.order_book.bids[dispatch.price] = \
                                self.order_book.bids[
                                    dispatch.price] * add + dispatch.size
                            if self.order_book.best_bid == dispatch.price:
                                self.order_book.best_bid_size = \
                                    self.order_book.bids[dispatch.price]
                        else:
                            self.order_book.bids[
                                dispatch.price] = dispatch.size
                elif dispatch.action == DispatchAction.ask:
                    if self.order_book.best_offer > dispatch.price or self.order_book.best_offer == 0:
                        ...
                        if dispatch.price in self.order_book.offers:
                            self.order_book.offers[dispatch.price] = \
                ...
                if dispatch.action == DispatchAction.bid:
                    if dispatch.price in self.order_book.bids:
                        if dispatch.nature == DispatchNature.cancel or \
                                self.order_book.bids[
                                    dispatch.price] == dispatch.size:
                            del self.order_book.bids[dispatch.price]
                            if self.order_book.best_bid == dispatch.price:
                                if self.order_book.bids:
                                    self.order_book.best_bid = max(
                                        self.order_book.bids.keys())
                                    self.order_book.best_bid_size = \
                                        self.order_book.bids[
                        ...
                        print("%s not in bids of %s" % (
                            dispatch.price, self.listing.code))
                elif dispatch.action == DispatchAction.ask:
                    if dispatch.price in self.order_book.offers:
                        if dispatch.nature == DispatchNature.cancel:
                            ...
                            if self.order_book.best_offer == dispatch.price:
                                if self.order_book.offers:
                                    self.order_book.best_offer = min(
                                        self.order_book.offers.keys())
                                    self.order_book.best_offer_size = \
                                        self.order_book.offers[
                                            self.order_book.best_offer]
                                else:
                        ...
                        print("%s not in offers of %s" % (
                            dispatch.price, self.listing.code))
        except Exception as e:
            self.logger.exception(
                "Error %s while updating order book for % s, dispatch nature is %s" % (
                    e, self.listing.code, dispatch.nature))

    async def subscribe(self, subs=None):
        if subs:
            if self.listing.exchange.code == 'BTM':
                request = {"op": "subscribe",
                           "args": ["orderBookL2" + self.listing.symbol]}
    ...
    def __parse_message(self, msg):
        ...
        if self.listing.exchange.code == 'BTM':
            ...
            if action == 'quote':
                b = Dispatch(self.listing.code, DispatchNature.quote,
                             DispatchAction.bid,
                             Decimal(str(msg['data'][0]['bidPrice'])),
                             Decimal(str(msg['data'][0]['bidSize'])))
                o = Dispatch(self.listing.code, DispatchNature.quote,
                             DispatchAction.ask,
                             Decimal(str(msg['data'][0]['askPrice'])),
                             Decimal(str(msg['data'][0]['askSize'])))
                return [b, o]
            elif action == 'trade':
                for trade in msg['data']:
                    ...
                    timestamp = datetime.datetime.strptime(trade['timestamp'],
                                                           '%Y-%m-%dT%H:%M:%S.%fZ').replace(
                        tzinfo=pytz.UTC)
                    dispatch = Dispatch(self.listing.code,
                                        DispatchNature.trade, action,
            ...
            if 'stream' in msg:
                if '{0}@trade'.format(self.listing.symbol) in msg['stream']:
                    ...
                    time = datetime.datetime.utcfromtimestamp(
                        msg['data']['T'] / 1000).replace(
                        tzinfo=pytz.UTC)  # binance posix in ms
                    dispatch = Dispatch(self.listing.code,
                                        DispatchNature.trade, action,
                                        Decimal(str(msg['data']['p'])),
                                        Decimal(str(msg['data']['q'])), time,
                                        msg['data']['b'], msg['data']['a'])
                    return [dispatch]
                elif '{0}@depth20'.format(self.listing.symbol) in msg[
                    'stream']:
                    self.order_book.empty()
                    ...
                    b = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.bid,
                                 Decimal(str(msg['data']['b'])),
                                 Decimal(str(msg['data']['B'])))
                    o = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.ask,
                                 Decimal(str(msg['data']['a'])),
                                 Decimal(str(msg['data']['A'])))
                    return [b, o]
            ...
            if 'events' in msg:
                for level in msg['events']:
                    if level['type'] == 'trade':
                        ...
                        dispatch = Dispatch(self.listing.code,
                                            DispatchNature.trade, action,
                                            trade_price, abs(size),
                                            msg['timestampms'] / 1000)
                        dispatches.append(
                            dispatch)  # returns a list of messages cause on init there are more than one order in the message
                        ...
                        if level['reason'] == 'initial' or level[
                            'reason'] == 'place':  # first msg with whole order book in it, only parse it if our own book is empty
                            dispatch = Dispatch(self.listing.code,
                                                DispatchNature.add, action,
                                                Decimal(str(level['price'])),
                                                abs(Decimal(
                                                    str(level['delta']))))
                        elif level['reason'] == 'cancel':
                            dispatch = Dispatch(self.listing.code, nature,
                                                action,
                                                Decimal(str(level['price'])),
                                                abs(Decimal(
                                                    str(level['delta']))))
                ...
                if msg['e'] == 'history-update':
                    for tick in msg['data'][::-1]:
                        if tick[0] == 'buy':
                            ...
                            action = DispatchAction.give
                        time = datetime.datetime.utcfromtimestamp(
                            int(tick[1]) / 1000).replace(
                            tzinfo=pytz.UTC)  # cex posix in ms
                        dispatch = Dispatch(self.listing.code,
                                            DispatchNature.trade, action,
                                            Decimal(tick[3]),
                                            abs(int(tick[2]) / 1e8), time,
                                            txid=tick[4])
                        dispatches.append(dispatch)
                    ...
                    b = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.bid,
                                 Decimal(str(msg['data']['buy'][0][0])),
                                 Decimal(str(msg['data']['buy'][0][1] / 1e8)))
                    o = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.ask,
                                 Decimal(str(msg['data']['sell'][0][0])),
                                 Decimal(str(msg['data']['sell'][0][1] / 1e8)))
                    return [b, o]
                elif msg['e'] == 'ping':
            ...
            if isinstance(msg, list):
                if 'hb' in msg:
                    ...
                    b = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.bid, Decimal(str(msg[1])),
                                 Decimal(str(msg[2])))
                    o = Dispatch(self.listing.code, DispatchNature.quote,
                                 DispatchAction.ask, Decimal(str(msg[3])),
                                 Decimal(str(msg[4])))
                    return [b, o]
                elif len(msg) == 2 and isinstance(msg[1],
                                                  list):  # snapshot of an order book
                    self.order_book.empty()
                    for level in msg[1]:
                        if level[2] > 0:  # bid
                            dispatch = Dispatch(self.listing.code,
                            ...
                            print(
                                "Bitfinex returning a snapshot of orderbook with an empty level")
                elif len(msg) == 4:  # update of an orderbook
                    if msg[2] == 0:
                        if msg[3] == 1:
                            dispatch = Dispatch(self.listing.code,
                                                DispatchNature.cancel,
                                                DispatchAction.bid,
                                                Decimal(str(msg[1])),
                                                Decimal('0.0'))
                            dispatches.append(dispatch)
    ...
    def is_alive(self):
        ...
        if self.startup_completed == False:
            ...
            if self.ws.state == CLOSING or self.ws.state == CLOSED:
                self.logger.warning(
                    "Connection lost, disconnected from %s" % self.listing.code)
                status = False
            else:
                status = True
        return status
    ...
    def update_flat_ob(self, dump_size=95000):
        t = datetime.datetime.utcfromtimestamp(self.order_book.time).replace(
            tzinfo=pytz.UTC)
        bids = [[k, v] for k, v in self.order_book.bids.items()]
        for i in range(len(bids)):
            b = bids[i]
            self.flat_ob += [
                {'price': float(b[0]), 'size': float(b[1]), 'timestamp': t,
                 'side': 'bid'}]
        ...
        if len(self.flat_ob) >= dump_size:
            ...
            print(
                'Flat order book of listing ' + self.listing.code + ' was loaded in MongoDB')
