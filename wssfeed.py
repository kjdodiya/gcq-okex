...
class WSSFeed(Feed):
    ...
    def __init__(self, listing, keyring=None, loop=None, capture_data=False, logger=None): #, lib_ob_flat=None):
        ...
        if self.listing.exchange.code == 'OKX' and self.listing.asset.type == 'CN':
    ...
    def __parse_message(self, msg):
        ...
        if self.listing.exchange.code == 'BTM':
            ...
            data = msg[0]['data'] if 'data' in msg[0] else None
            if data is not None:
                if 'asks' in data:
                    ...
                    for level in data['asks']:
                        dispatch = Dispatch(self.listing.code, DispatchNature.refresh, DispatchAction.ask, Decimal(str(level[0])), Decimal(str(level[1])))
                        dispatches.append(dispatch)
                if 'bids' in data:
                    ...
                    for level in data['bids']:
                        dispatch = Dispatch(self.listing.code, DispatchNature.refresh, DispatchAction.bid, Decimal(str(level[0])), Decimal(str(level[1])))
                        dispatches.append(dispatch)
                else:
                    if 'result' in data:
                        ...
                        for trade in data:
                            if trade[4] == 'ask':
                                ...
                                action = DispatchAction.lift
                            timestamp = datetime.datetime.combine(datetime.date.today(), datetime.datetime.strptime(trade[3], '%H:%M:%S').time()).replace(tzinfo=pytz.UTC) - datetime.timedelta(hours=8, minutes=0)

                            dispatch = Dispatch(self.listing.code, DispatchNature.trade, action, trade[1], trade[2], timestamp)
                            dispatches.append(dispatch)
