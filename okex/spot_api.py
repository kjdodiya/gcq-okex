...
class SpotAPI(Client):
    ...
    #async def get_ledger_record_paging(self, symbol, before, after, limit):
    #    params = {'before': before, 'after': after, 'limit': limit}
    #    return self._request_with_params(GET, SPOT_LEDGER_RECORD + str(symbol) + '/ledger', params, cursor=True)

    # take order
    async def take_order(self, otype, side, instrument_id, size, margin_trading=1, client_oid='', price='', funds='',order_type = ''):
    ...
    # ]
    async def take_orders(self, params):
    ...
    # ]
    async def revoke_orders(self, params):
    ...
    async def get_orders_list(self, status, instrument_id, froms='', to='', limit='100'):
    ...
    async def get_order_info(self, instrument_id='btc-usdt',order_id='',client_oid=''):
        params = {'instrument_id': instrument_id}
        if order_id:
            return await self._request_with_params(GET, SPOT_ORDER_INFO + str(order_id), params)
        elif client_oid:
            return await self._request_with_params(GET, SPOT_ORDER_INFO + str(client_oid), params)

    async def get_orders_pending(self, froms, to, limit, instrument_id):
        params = {}
        if froms:
            params['from'] = froms
        if to:
        ...
        if instrument_id:
            params['instrument_id'] = instrument_id
        return await self._request_with_params(GET, SPOT_ORDERS_PENDING, params, cursor=True)

    async def get_fills(self, order_id, instrument_id, froms, to, limit):
    ...
    # query depth
    async def get_depth(self, instrument_id, size='', depth=''):
        params = {}
        if size:
            params['size'] = size
        if depth:
    ...
    async def get_specific_ticker(self, instrument_id):
        return await self._request_without_params(GET, SPOT_SPECIFIC_TICKER + str(instrument_id) + '/ticker')

    async def get_deal(self, instrument_id, froms, to, limit):
        params = {}
        if froms:
            params['from'] = froms
        if to:
        ...
        if limit:
            params['limit'] = limit
        return await self._request_with_params(GET, SPOT_DEAL + str(instrument_id) + '/trades', params)

    # query k-line info
    async def get_kline(self, instrument_id, start, end, granularity):
        params = {}
        if start:
            params['start'] = start
        if end:
